...
const LastWord = (str) => {
  let spaceIndex=str.lastIndexOf(' ') ;
   return str.slice(spaceIndex+1, str.length);
}
...
const LastWord_2  = (str) => {
    ...
    let lastWord=sepStr.slice(sepStr.length - 1);
...
for (let i=0;i<sepStr.length;i++){
  if (sepStr[i] == 'I'){
    sepStr.splice(sepStr.indexOf('I'), 1 , 'We');
  }
   if (sepStr[i] == 'am') {
    sepStr.splice(sepStr.indexOf('am'), 1 , 'are');
    }
  if (sepStr[i] == 'was') {
    sepStr.splice(sepStr.indexOf('was'), 1 , 'were'); 
  }
}
  return sepStr.join(' ');
}
